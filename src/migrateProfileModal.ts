...
class MigrateModal extends Modal {
	...
	private createMigrateSettingsSection(
		title: string,
	...
	private async updateVaultItemsList(isDirectory: boolean): Promise<void> {
		...
		const vaultItems = isDirectory
			? this.plugin.settings.vaultDirs
			: this.plugin.settings.vaultFiles;
		const itemNames = items
			.filter((item) =>
				isDirectory
					? item.isDirectory()
					: item.isFile() && path.extname(item.name) === '.json'
			)
	...
	private getDefaultFileState(fileName: string): boolean {
		...
		const optionalFiles = ['core-plugins-migration', 'plugins', 'community-plugins'];
	...
	private getThemeCount(): number {
		try {
			...
			return items.filter((item) => item.isFile() && item.name.endsWith('.css'))
				.length;
		} catch {
			return 0;
		}
	...
	private togglePluginSelectionButton(
		setting: Setting,
		isEnabled: boolean,
