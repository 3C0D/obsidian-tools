...
export function registerVaultContextMenu(): void {
	...
	fileExplorerView = this.app.workspace.getLeavesOfType(
		InternalPluginName.FileExplorer
	)[0].view;
	...
	const vaultSwitcherEl = document.querySelector<HTMLElement>(
		'.workspace-drawer-vault-switcher'
	);
	if (
		vaultSwitcherEl &&
		fileExplorerView.files &&
		!fileExplorerView.files.has(vaultSwitcherEl)
	) {
		...
		this.registerDomEvent(
			...
			async (ev: MouseEvent): Promise<void> => {
				fileExplorerView.openFileContextMenu(
					ev,
					vaultSwitcherEl.childNodes[0] as HTMLElement
				);
				document.body.click();
			}
		);
...
function handleFileMenuEvent(menu: Menu, file: TAbstractFile): void {
	...
	menu.items = menu.items.filter(
		(item) => !localizedTitles.includes(item.titleEl?.textContent ?? '')
	);
	...
	if (plugin && plugin.settings['delete-folders-by-name']) {
		menu.addSeparator();
		menu.addItem((item) => {
			item.setTitle('Delete folders by name')
				.setIcon('trash-2')
