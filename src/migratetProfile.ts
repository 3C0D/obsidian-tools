...
export async function migrateProfile(
	...
	try {
		const sourceOrDest = customPath
			? path.join(customPath, '.obsidian')
			: await getVaultDirPath('.obsidian', isImport);
		if (!sourceOrDest) return;

		const action = isImport ? 'Import' : 'Export';
		const res = await openMigrateModal(
			plugin,
			app,
			sourceOrDest,
			`${action} vault options`,
			isImport
		);
...
async function getVaultDirPath(
	complement: string,
	isImport = true
): Promise<string | null> {
...
async function migrateVaultDirectories(
	plugin: Tools,
	app: App,
	dirPath: string,
	isImport = true
...
async function migrateVaultJsonFiles(
	plugin: Tools,
	app: App,
	dirPath: string,
	isImport = true
	...
	for (const [key, isEnabled] of Object.entries(vaultFiles)) {
		...
		const sourcePath = isImport
			? path.join(dirPath, `${key}.json`)
			: path.join(obsidian, `${key}.json`);
		const destinationPath = isImport
			? path.join(obsidian, `${key}.json`)
			: path.join(dirPath, `${key}.json`);
