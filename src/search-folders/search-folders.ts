...
class FolderListModal extends FuzzySuggestModal<TFolder> {
	constructor(
		...
		super(app);
		this.setPlaceholder(
			`Folders named "${folderName}" (double-click to reveal in explorer)`
		);
	}
	...
	onChooseItem(folder: TFolder, evt: MouseEvent | KeyboardEvent): void {
		...
		if (fileExplorer && fileExplorer.view) {
			// @ts-expect-error - The fileExplorer.view.revealInFolder method exists but is not in the type definitions
