...
function readObsidianConfig(): ObsidianJsonConfig | null {
	try {
		...
		if (vaultsConfigPath) {
			const vaultsConfigContent = JSON.parse(
				readFileSync(vaultsConfigPath, 'utf8')
			);
...
export function getVaultPaths(app: App): string[] {
	...
	if (obsidianConfig) {
		...
		for (const key in obsidianConfig.vaults) {
			if (
				Object.prototype.hasOwnProperty.call(obsidianConfig.vaults, key) &&
				obsidianConfig.vaults[key].path !== currentVaultPath
			) {
