...
export class PluginSelectionModal extends Modal {
	...
	private async createPluginSetting(
		...
		if (
			...
			if (plugin.conflictReason === 'Lower version') {
				titleParts.push(
					`• <span style="color: orange;">${plugin.conflictReason}</span>`
				);
			} else {
		...
		const setting = new Setting(container).addToggle((toggle) => {
			toggle.setValue(defaultSelected).onChange((value) => {
		...
		try {
			...
			for (const dir of pluginDirs) {
				...
				if (conflictInfo || devConflictInfo) {
					...
					pluginInfo.conflictReason =
						conflictInfo || devConflictInfo || undefined;
					this.conflictCount++;
				}
	...
	private getDestinationPluginsPath(): string {
		if (this.isImport) {
			...
			return (
				this.destinationPluginsPath ||
				this.app.vault.adapter.getFullPath('.obsidian/plugins')
			);
		}
	}
